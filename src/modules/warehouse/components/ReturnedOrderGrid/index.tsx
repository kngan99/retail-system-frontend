...
const ReturnedOrderGrid = (props: ComponentProps) => {
          ...
          <Row>
            {title && (
              <Col xs={12}>
                <h3 className="block-title">
                  {title ? title : "Return Products Requests Manage"}
                </h3>
              </Col>
            )}
            <Col xs={12} className="block-content">
              <Table responsive="lg">
                ...
                <tbody>
                  {cartStore.returnedOrders.map((item: any, index: number) => (
                    <tr key={item.Id}>
...
export default observer(ReturnedOrderGrid);
