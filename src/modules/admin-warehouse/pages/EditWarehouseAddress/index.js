...
const EditWarehouseAddressPage = () => {
  ...
  let warehouseID = window.location.pathname.split("/")[3];
  ...
  const handleChangeAddress = async () => {
    const res = await warehouseStore.setAddress(
      warehouseID,
      chosenAddress,
      Lat,
      Lng
  ...
  React.useEffect(() => {
    warehouseID = window.location.pathname.split("/")[3];
    const res = warehouseStore.getAccountById(warehouseID);
    setState({
      count: 0,
  ...
  return (
    <>
      ...
      <AdminWrapper pageTitle={"Edit Warehouse Address"}>
...
export default observer(EditWarehouseAddressPage);
