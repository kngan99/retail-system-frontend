...
interface ReturnedCargoRequest {
  ProductId: number[];
  Quantity: number[];
  Status?: string | null;
...
class CartStore {
  ...
  @observable currentCustomer: any = {
    Id: 0,
    Phone: "",
    Address: "",
    City: "",
  ...
  @observable returnedCargoRequest: ReturnedCargoRequest = {
    ProductId: [],
    Quantity: [],
    CargoRequestId: -1,
  };
  ...
  @observable returnedOrders: any[] = [];
  @observable returnedSelectedOrder: any = null;
  @observable totalReturnedOrders: number = 0;
  ...
  getTransactions = async () => {
    ...
    for (let i = 0; i < res[1]; i++) {
      ...
      for (this.j = 0; this.j < res[0][i].Products[1]; this.j++) {
        this.transactionStrOneOrderProductsName.push(
          res[0][i].Products[0][this.j].Product.ProductName
        );
      }
      this.transactionArrayName.push(
        this.transactionStrOneOrderProductsName.join(", ")
      );
      this.j = 0;
    ...
    if (product.Discontinued && !isImport) {
      ...
      await this.productsInCart.map((item) => {
        if (item.Id === product.Id) {
          ...
          item.Total = Number(
            (
              (item.UnitPrice * item.Quantity * (100 - item.Discount)) /
              100
            ).toFixed(2)
          );
          found = true;
      ...
      if (!found) {
        await this.productsInCart.push({
          ...product,
          Quantity: 1,
          RawTotal: product.UnitPrice,
          Total: Number(
            ((product.UnitPrice * (100 - product.Discount)) / 100).toFixed(2)
          ),
        });
      }
    }
  ...
  getCashierInfo = async () => {
    ...
    if (result && result.Salesclerk) {
      this.salescleckFullName =
        result.Salesclerk.FName + " " + result.Salesclerk.LName;
  ...
  confirmStripeOrder = async (stripe: string) => {
    const result = await orderService.confirmStripeOrder(
      this.salescleckId,
      this.session,
      this.productsInCart,
      this.currentCustomer.Id,
  ...
  confirmVnpayOrder = async (vnpay: string) => {
    const result = await orderService.confirmVnpayOrder(
      this.salescleckId,
      this.session,
      this.productsInCart,
      this.currentCustomer.Id,
  ...
  @action.bound
  sendReturnedCargoRequest = async () => {
    const addedUserCargoRequest = {
      ...this.returnedCargoRequest,
      UserId: retrieveFromStorage("loggedId"),
    };
    console.log(addedUserCargoRequest);
    const result = await cartService.createReturnCargoRequest(addedUserCargoRequest);
    return result;
  };
  ...
  @action.bound
  async updateStatusReturnedCargoReq(id: number, currentStatus: string) {
    let nextStatus = currentStatus;
    if (currentStatus === "Created") {
      nextStatus = "Confirmed";
    } else if (currentStatus === "Confirmed") {
      nextStatus = "Shipper Coming";
    } else if (currentStatus === "Shipper Coming") {
      nextStatus = "Picked";
    } else if (currentStatus === "Picked") {
      nextStatus = "Success";
    }
    const data = await cartService.updateStatusReturnedCargoReq(id, nextStatus);
    return data;
  }
  ...
  @action.bound
  async getReturnedCargoReqStatus(id: number) {
    const res = await cartService.getReturnedCargoReqStatus(id);
    return res.data;
  }
  ...
  async getReturnedOrderById(id: number) {
    const data = await cartService.getReturnedOrderById(id);
    this.selectedOrder = data;
    return data;
  }
  ...
  async getReturnedOrdersByOrderId(id: number) {
    this.returnedOrders = [];
    const data = await cartService.getReturnedOrdersByOrderId(id);
    data[0].map((x) => this.returnedOrders.push(x));
    this.totalReturnedOrders = data[1];
    return data[0];
  }

  @action.bound
  async setReturnedOrderById(id: number) {
    const data = await cartService.getReturnedOrderById(id);
    this.productsInCart = [];
    let newProductInCart: CartProduct = {
        Id: -1,
        ProductName: '',
        CategoryId: -1,
        QuantityPerUnit: '',
    ...
    data.products.map((x, index) => {
      newProductInCart.Id = x.Id;
      newProductInCart.ProductName = x.ProductName;
      newProductInCart.UnitPrice = data.quantities[index]; //quantity
      newProductInCart.ReorderLevel = data.returned[index]; //returned
      newProductInCart.Quantity = 0;
      this.productsInCart.push(newProductInCart);
      for (var key in newProductInCart ) {
        newProductInCart[key] = null;
      }
    });
  ...
  async setProductsInCartQuantity() {
    const result = await this.selectedOrder.quantities.map((item, idx) => {
      this.productsInCart[idx].Quantity = item;
    });
    console.log(this.productsInCart);
    return result;
  }
  ...
  async setProductsInCartTotal() {
    const result = await this.selectedOrder.quantities.map((item, idx) => {
      this.productsInCart[idx].Total =
        item * this.productsInCart[idx].UnitPrice;
    });
    return result;
  }
